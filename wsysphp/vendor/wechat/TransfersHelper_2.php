 ...
 * 
 */
class TransfersHelper extends \WeiXinPay
{
    private $parameters;
    private $uniacid;
    private $key;

    function __construct($param, $uniacid, $key)
    {
...
//            $this->parameters["client_ip"] &&
    ...
    {
        // $this->key = 'awkcgogjiaiucyvu4kuruo34lr2cybuc';
        if (!$this->key) {
            die('密钥不能为空');
        }
...
//        }
        ...
        return $this->sign($unSignParaString,$this->key);
    }
    ...
    private function getPem() {
        global $_W;
        $unisetting = pdo_get('uni_settings', array('uniacid' => $this->uniacid), array('payment'));
        ...
        $certPath = $wsysCachePath . '/wsys_cert_uniacid'.$this->uniacid.'.pem';
        $keyPath  = $wsysCachePath . '/wsys_key_uniacid'.$this->uniacid.'.pem';
